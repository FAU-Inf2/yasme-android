...
//Tabelle fuer Chats --> KeyID noetig
//Tabelle fuer KeyID --> Key noetig
...
//WICHTIG: Bevor Nachrichten vom Server geholt werden, muessen neue Keys vom Server geholt werden und diese Tabellen aktualisiert werden
...
public class MessageEncryption {
	...
	public MessageEncryption(Context context, Id chatid, long creator, long recipient, long devid){
		...
		if (!sharedPref.contains(Long.toString(chatid.getId()))){
			...
			//###DEBUG
			System.out.println("[???]: KeyID "+keyid.getId()+" f�r Chat "+chatid.getId() + " wurde erstellt und gespeichert");
		...
		//if old key is already available
			...
			if(base64arr != null){
				...
				//###DEBUG
				System.out.println("[???]: Key "+ keyid.getId()+" f�r Chat "+ chatid.getId()+" wurde geladen");
				///###
